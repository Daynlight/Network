...
void register_user(char *decompressed_data, int i){
  char name[NAMESIZE];
...
void private_message(char *decompressed_data, int i){
  char data[NAMESIZE + BUFFER_SIZE] = {0};
      ...
      if(clients.clientData[i].socket_id > 0){
        ...
        default:
          ...
          if(decompressed_data[0] == '1'){       // Register
            register_user(decompressed_data, i);
          }
          else if(decompressed_data[0] == '2'){  // Message
            public_message(decompressed_data, i);
          }
          else if(decompressed_data[0] == '3'){  // Private Message
            private_message(decompressed_data, i);
          };
        break;
        };
      };
